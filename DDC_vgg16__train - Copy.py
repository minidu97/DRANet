...
def main(args):
    ...
    for iter in range(1, iterations+1):
        ...
        save_name = './predictions/DDC_' + source_dataset + '_to_' + target_dataset + '_iter' + str(iter) + '_predictions.pt'
    ...
    # Extract the average test accuracies from the tuples in test_acc_src
    test_acc_values = [acc[0] for acc in test_acc_src]

    # Calculate the mean and standard deviation manually
    test_acc_avg = sum(test_acc_values) / len(test_acc_values)
    test_acc_var = (sum((x - test_acc_avg) ** 2 for x in test_acc_values) / len(test_acc_values)) ** 0.5

    print("Source average test acc: {:.4f} | Standard deviation test: {:.4f}".format(test_acc_avg, test_acc_var))


    print('Target test_acc=', test_acc)
    # Extract the average test accuracies from the tuples in test_acc
    test_acc_values = [acc[0] for acc in test_acc]

    # Calculate the average test accuracy
    test_acc_avg = sum(test_acc_values) / len(test_acc_values)

    print('Target test_acc_avg:', test_acc_avg)
    print("The End")
