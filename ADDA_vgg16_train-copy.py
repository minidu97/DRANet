...
def test(encoder, classifier, dataloader_test, dataset_size_test):
    ...
    print()
    torch.cuda.empty_cache()
    return avg_acc, y_true, y_pred, y_prob
...
def main(args):
    ...
    for iter in range(1,iterations+1):
        ...
        # Calculate average test accuracy for source domain
        #test_acc_avg_src = sum(acc for acc, _, _, _ in test_acc_src) / len(test_acc_src)

        # Calculate average test accuracy for target domain
        #test_acc_avg_tgt = sum(acc for acc, _, _, _ in test_acc) / len(test_acc)


        # Save predictions and labels
        save_name = './predictions/ADDA_' + source_dataset + '_to_' + target_dataset + '_iter' + str(iter) + '_predictions.pt'
    ...
    # Calculate average test accuracy for source domain
    test_acc_avg_src = sum(acc for acc, _, _, _ in test_acc_src) / len(test_acc_src)
    # Calculate standard deviation of test accuracy for source domain
    test_acc_var_src = statistics.stdev(acc for acc, _, _, _ in test_acc_src)

    # Calculate average test accuracy for target domain
    test_acc_avg_tgt = sum(acc for acc, _, _, _ in test_acc) / len(test_acc)
    # Calculate standard deviation of test accuracy for target domain
    test_acc_var_tgt = statistics.stdev(acc for acc, _, _, _ in test_acc)

    # Print results for source domain
    print('Source  test_acc=', test_acc_src)
    print("Source average test acc: %.4f" % test_acc_avg_src, '| Variance test: %.4f' % test_acc_var_src)

    # Print results for target domain
    print('Target test_acc=', test_acc)
    print("Target Average test acc: %.4f" % test_acc_avg_tgt, '| Variance test: %.4f' % test_acc_var_tgt)
