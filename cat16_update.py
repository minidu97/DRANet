...
def cat_train(encoder, classifier, netD, dataloader_train_s, dataloader_train_t, epochs):
    ...
    for epoch in range(epochs):
        ...
        for i in range(num_iter):
            ...
            data_source, label_source = next(iter(iter_source))
            data_target, _ = next(iter(iter_target))
...
def main(args):
    ...
    for iter in range(1,iterations+1):
        ...
        src_acc, true_labels_s, pred_labels_s, pred_scores_s = test(src_encoder, src_classifier, dataloader_s[TEST_S],
                                                                     dataset_sizes_src[TEST_S])
        test_acc_src.append(src_acc)
        true_labels_src.extend(true_labels_s)
        pred_labels_src.extend(pred_labels_s)
        pred_scores_src.extend(pred_scores_s)

        print("Test scr_encoder + src_classifier on Target Test dataset")
        tgt_acc, true_labels_t, pred_labels_t, pred_scores_t = test(src_encoder, src_classifier, dataloader_t[TEST_T],
                                                                    dataset_sizes_tgt[TEST_T])
        test_acc.append(tgt_acc)
        true_labels_tgt.extend(true_labels_t)
        pred_labels_tgt.extend(pred_labels_t)
        pred_scores_tgt.extend(pred_scores_t)
    ...
    precision_src = precision_score(true_labels_src, pred_labels_src, average='macro')
    recall_src = recall_score(true_labels_src, pred_labels_src, average='macro')
    auc_src = roc_auc_score(true_labels_src, pred_scores_src, average='macro', multi_class='ovo')

    precision_tgt = precision_score(true_labels_tgt, pred_labels_tgt, average='macro')
    recall_tgt = recall_score(true_labels_tgt, pred_labels_tgt, average='macro')
    auc_tgt = roc_auc_score(true_labels_tgt, pred_scores_tgt, average='macro', multi_class='ovo')

    # Print Precision, Recall, and AUC
    print('Source Precision:', precision_src)
    print('Source Recall:', recall_src)
    print('Source AUC:', auc_src)

    print('Target Precision:', precision_tgt)
    print('Target Recall:', recall_tgt)
    print('Target AUC:', auc_tgt)
