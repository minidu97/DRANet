...
def main(args):
    ...
    for iter in range(1,iterations+1):
        ...
        if enable_transfer:
            ...
            save_name = 'predictions/ADDA_EM_' + source_dataset + '_to_' + target_dataset + '_iter' + str(iter) + '_predictions.pt'
    ...
    if enable_transfer:
        ...
        # Extract only the accuracy values from the list of tuples
        test_acc_values = [acc[0] for acc in test_acc]
        test_acc_avg = sum(test_acc_values) / len(test_acc_values)
        test_acc_var = statistics.stdev(test_acc_values)
